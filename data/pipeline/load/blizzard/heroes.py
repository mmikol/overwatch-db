...
skipped; perks are included. No lore, no media URLs.
    ...
    python -m data.pipeline.load.blizzard.heroes --dsn postgresql://user@localhost/overwatch
    DATABASE_URL=... python -m data.pipeline.load.blizzard.heroes
"""
...
from data.pipeline.ingest.blizzard import BASE_URL, HEROES_URL, USER_AGENT
from data.pipeline.extract.blizzard.heroes import (
    parse_abilities,
    parse_perks,
    parse_roster,
    parse_subroles,
...
def main():
    ...
    roster_soup = BeautifulSoup(ingest.cached_get(session, HEROES_URL, args.cache,
                            ingest.cache_key(HEROES_URL)), "html.parser")
    ...
    for index, hero in enumerate(heroes, start=1):
        ...
        page = ingest.cached_get(
            session, url, args.cache, ingest.cache_key(slug)
        )
